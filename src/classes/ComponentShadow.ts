...
export function createShadow<Info extends Partial<ComponentInfo> = {}>(func: ComponentFuncNoCtxShortcut | ComponentFuncWithCtxShortcut | ComponentType, signals?: Partial<ComponentShadowSignals> | null, name: string = func.name) {
    // Exceptionally we also support feeding in a class here. To add support for being a shadow.
    const Shadow = func["MIX_DOM_CLASS"] ? { [name]: class extends (func as ComponentType) {} }[name] as ComponentShadowType<Info> : createComponent(func as ComponentFuncNoCtxShortcut<Info> | ComponentFuncWithCtxShortcut<Info>, name) as ComponentShadowFuncWith<Info>;
